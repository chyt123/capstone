...
/**
 * Created by ycheng on 9/21/17.
 */
public class LDATester implements AlgorithmTester {
    ...
    private static int[] core(IdSet<Integer> idSet, Recommender recommender, int limit, double sourceDataRatio, int threshold) {
        ...
        try {
            int[] stat = recommender.recommendTest(idSet, limit, (int) (size * sourceDataRatio), threshold, "lda");
            return stat;
        } catch (Exception e) {
            e.printStackTrace();
            return null;
